...
import math
from spatial_bicycle_models3 import BicycleModel
...
def readVideo():
    ...
    inpImage = cv2.VideoCapture(os.path.join(CWD_PATH, 'keliling.avi'))
...
def processImage(inpImage):
    ...
    lower_white = np.array([0, 100, 30]) #lower_black
    upper_white = np.array([25, 140, 80]) #lower_black
    ...
    ret, thresh = cv2.threshold(gray, 20, 50, cv2.THRESH_BINARY) ##
...
def perspectiveWarp(inpImage):
    ...
    src = np.float32([[270, 150],#atas kiri
                      [550, 150],#atas kanan
                      [0, 469],#bawah kiri
                      [820, 469]])#bawah kanan
    height = 616
...
#                      [299, 600],
#                      [499, 600]])
    dst = np.float32([[0, 0],#atas kiri
                      [width-1, 0],#atas kanan
                      [260, height-1],#bawah kiri
                      [width-261, height-1]])#bawah kanan
...
#image = readVideo()
#image = cv2.imread('output20210607-152505pic0-Copy1.jpg')
image = cv2.VideoCapture(gstreamer_pipeline(flip_method=0), cv2.CAP_GSTREAMER)
#cap=cv2.VideoCapture('drive.mp4')
frame_width = int(image.get(3))
frame_height = int(image.get(4))
timestr = time.strftime("%Y%m%d-%H%M%S")
out = cv2.VideoWriter('output'+timestr+'.avi',cv2.VideoWriter_fourcc(*'mp4v'), 
                      10, (frame_width,frame_height))
##############
# Controller start #
##############
model = BicycleModel(length=0.153, width=0.167)

N = 4
Q = sparse.diags([1.0, 0.0, 0.0])
R = sparse.diags([0.5, 0.0])
QN = sparse.diags([1.2, 0.0, 0.0])

v_max = 1.0  # m/s
delta_max = 0.2499*(1/0.7)  # rad
ay_max = 0.5  # m/s^2
...
start = time.time()
car.steering_gain = 1
car.throttle_gain = 0.9
v_ref= 0.18
#car.throttle=0.9
yy = [0*ym_per_pix, 199*ym_per_pix, 399*ym_per_pix, 599*ym_per_pix]
delta_y = yy[1]-yy[0]
while True:
...
#    ret = True
    ...
    if time.time() - start >=0.05:
            ...
            #    centerline = meanPts[0][:,0]
            #    plt.plot(left_fitx,  ploty, color = 'yellow')
            #    plt.plot(right_fitx, ploty, color = 'yellow')
            #    plt.xlim(0, 720)
            #    plt.ylim(600, 0)
            #    plt.plot(np.flip(meanPts[0][:,0]), ploty, color = 'red')
            #    plt.show()
            ...
            #        xx = [(meanPts[0][599,0]-310)*xm_per_pix, (meanPts[0][399,0]-310)*xm_per_pix, 
            #             (meanPts[0][199,0]-310)*xm_per_pix, (meanPts[0][0,0]-310)*xm_per_pix]
            xx = [(meanPts[0][0,0]-411)*xm_per_pix, (meanPts[0][199,0]-411)*xm_per_pix, 
	             (meanPts[0][399,0]-411)*xm_per_pix, (meanPts[0][599,0]-411)*xm_per_pix]
            #"""
            #"""
            curve = curveRad
            ...
            mpc.reff(delta_y, 1/curveRad, v_ref, xx)
            mpc.get_control()
            ...
            #car.steering = delta / 0.33
            #car.throttle= 0.4
            
            if mpc.current_control[0] >=1.0:
	            car.throttle = 1.0
            else :
	            car.throttle = mpc.current_control[0]/v_max
            
            #if mpc.current_control[1] <=0.22:
	    #        car.steering = 0.82793
            #elif mpc.current_control[1] >=-0.22:
	    #        car.steering = -1
            #else :
	    #        car.steering = mpc.current_control[1]

            
            if mpc.current_control[1] <= 0.0 :
	            car.steering = mpc.current_control[1]/delta_max *0.82793
            elif mpc.current_control[1] > 0.0:
	            car.steering = mpc.current_control[1]/delta_max
            ...
            if delta >=delta_max:
	            ...
	            car.steering = delta/delta_max*0.82793
            elif delta >0.0 :
	            car.steering = delta/delta_max
            """
            
            start = time.time()
            #print(a, car.steering, mpc.current_control[1])
            a+=1
            print(xx)
            ##Print to text file
            file.write("t = " + str(a*0.05) + ", delta = " + str(delta) + ". \n")
