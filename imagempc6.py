...
import math
from Model2 import BicycleModel
from Kontroller2 import MPC
...
ym_per_pix = 3.9 / 480 #mengasumsikan 3,9 meter mewakili 480 pixel
xm_per_pix = 1.6 / 640 #mengasumsikan 1,6 meter mewakili 640 pixel
class ImprovedLaneDetection:
    def __init__(self):
        ...
        with open(self.csv_filename, 'w', newline='') as file:
            ...
            writer.writerow(['Frame', 'Kecepatan', 'Y0X_Pixel', 'Y239X_Pixel', 'Y479X_Pixel', 'errorY_0_X', 'errorY_239_X', 'errorY_479_X', 'Curvature', 'xx (meter)', 'yy(meter)', 'Steering_Angle', 'Angle', 'error_yaw','FPS'])
    ...
    def calculate_curvature(self, leftx, lefty, rightx, righty):
        ...
        # Balik urutan agar perhitungan kurvatur benar
        leftx = leftx[::-1]
        lefty = lefty[::-1]
        rightx = rightx[::-1]
        righty = righty[::-1]
        ...
        # Hitung kurvatur dengan rumus: R = (1 + (2Ay + B)^2)^(3/2) / |2A|
...
def ModelPredictiveControl ():
    ...
    # Pastikan v_ref adalah angka tunggal
    v_ref = 0.18  # Sesuaikan dengan skala kecepatan yang digunakan
    if isinstance(v_ref, (list, np.ndarray)):
        v_ref = float(v_ref[0])

    v_ref = max(v_ref, 1e-6)  # Hindari pembagian dengan nol
...
def main():
    ...
    frame_count = 0
    mpc, delta_s, v_ref, yy = ModelPredictiveControl()
    ...
    while True:
        try :
            ...
            curvature = detector.calculate_curvature(leftx, lefty, rightx, righty) #calculate curvature
            ...
            if mpc_active and len(xx) == 3:
                #mpc.kom(delta_s, 1 / curvature if curvature != 0 else 0, xx) #kode imagecontrol6.py
                mpc.reff(delta_s, 1 / curvature if curvature != 0 else 0, v_ref, xx)
                mpc.get_control()
                ...
                #mpc_control0 = np.clip(mpc.current_control[0], -20.95, 20.95) #ini kecepatan
                mpc_control1 = np.clip(mpc.current_control[1], -20.95, 20.95) #ini steering angle

                # Ubah ke rentang 0° - 40°
                steering_angle_deg = ((mpc_control1 + 20.95) / 41.9) * 13

                #print(f"Sudut kemudi: {mpc.current_control[0]:.2f} derajat")
                print(f"Kecepatan : {mpc.current_control[0]}")
            ...
            with open(detector.csv_filename, 'a', newline='') as file:
                ...
                writer.writerow([
                    ...
                    xx[0], xx[1], xx[2], 
                    f"{(1/curvature_value):.2f}" if curvature_value != 0 else "0.00", 
                    xx, yy, steering_angle_deg, angle, error_yaw, f"{fps:.1f}"
                ])
            ...
            cv2.putText(curve_img, f"Kecepatan MPC: {mpc.current_control[0]}", (50, 150), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
