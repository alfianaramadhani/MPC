...
#ym_per_pix = 2.1 / 480 #mengasumsikan 1,6 meter mewakili 480 pixel
#xm_per_pix = 2.1 / 640 #mengasumsikan 3,9 meter mewakili 640 pixel

ym_per_pix = 3.3 / 480 #mengasumsikan 1,6 meter mewakili 480 pixel
xm_per_pix = 2.2 / 640 #mengasumsikan 3,9 meter mewakili 640 pixel
...
class ImprovedLaneDetection:
    def __init__(self):
        #self.tl, self.tr, self.br, self.bl =  [162, 159], [520, 157], [629, 279], [4, 322] #kamera 1m 
        #self.tl, self.tr, self.br, self.bl =  [119, 237], [521, 237], [631, 445], [6, 435] #video 4.mp4
        #self.tl, self.tr, self.br, self.bl =  [118, 197], [473, 198], [588, 361], [6, 361]
        self.tl, self.tr, self.br, self.bl =  [80, 38], [518, 39], [624, 203], [2, 202]
        ...
        with open(self.csv_filename, 'w', newline='') as file:
            ...
            writer.writerow(['Frame', 'Kecepatan', 'Y0X_Pixel', 'Y239X_Pixel', 'Y479X_Pixel', 'Y0X_Meter', 'Y239X_Meter', 'Y479X_Meter', 'errorY_0_Xmeter', 'errorY_239_Xmeter', 'errorY_479_Xmeter','Radius', 'Curvature', 'Y0(meter)', 'Y239(meter)', 'Y479(meter)','Error_Lateral','Error_Yaw (rad)','Error_Yaw(deg)','Angle_MPC(rad)', 'Angle_MPC(deg)','Konversi_MPC2Angle', 'Angle','FPS', 'km/h'])
...
def main():
    ...
    while True:
        try :
            ...
            e_psi_deg = np.degrees(mpc.model.spatial_state.e_psi)
            ...
            with open(detector.csv_filename, 'a', newline='') as file:
                ...
                writer.writerow([
                    ...
                    1/curvature[0],
                    yy[0],yy[1],yy[2], 
                    mpc.model.spatial_state.e_y, mpc.model.spatial_state.e_psi, #error lateral, error yaw (radian)
                    e_psi_deg, #error yaw derajat (tidak dikonversi)
