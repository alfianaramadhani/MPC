...
import math
from Model3 import BicycleModel
from Kontroller3 import MPC
...
def main():
    global steering_angle_deg, angle, kec, mpc_active, start_flag #purepersuit_active
    ...
    while True:
        try :
            ...
            if mpc_active and len(xx) == 3:
                ...
                #mpc.kom(delta_s, 1 / curvature, xx) 
                ...
                #mpc_control0 = np.clip(mpc.current_control[0], -20.95, 20.95) #kecepatan
                ...
                steering_angle_deg = ((mpc_control1 + 20.95) / 41.9) * 19 #karena angle aku setting di 21 derajat, maka dikalikan 19 (40-21 = 19)

                print(f"Sudut kemudi: {mpc.current_control[0]:.3f} m/s")
            ...
            #cv2.putText(curve_img, f"Curvature: {1 / curvature if curvature != 0 else 0}", (50, 150), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
