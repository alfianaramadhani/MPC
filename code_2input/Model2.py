...
#    def __init__(self, reference_path, length, width, Ts):
    ...
    def linearize(self, v_ref, kappa_ref, delta_s):
        ...
        # Pastikan v_ref adalah angka tunggal, bukan list atau array
        ...
        # Pastikan tipe data semua elemen dalam array adalah float
        a_1 = np.array([1.0, delta_s, 0.0])
        a_2 = np.array([-kappa_ref ** 2 * delta_s, 1.0, 0.0])
        a_3 = np.array([-kappa_ref / v_ref * delta_s, 0.0, 1.0])

        b_1 = np.array([0.0, 0.0])
        b_2 = np.array([0.0, delta_s])
        b_3 = np.array([-1.0 / (v_ref ** 2) * delta_s, 0.0])

        f = np.array([0.0, 0.0, 1.0 / v_ref * delta_s])
